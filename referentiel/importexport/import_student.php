...
* D'apres competency/import_student.php 
*
* @package referentiel
*/
	...
	elseif ($id) {
		...
		error(get_string('erreurscript','referentiel','Erreur01 : import_student.php'));
	}
...
/*
    	...
    	groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/referentiel/export_task.php?d='.$referentiel->id.'&amp;mode='.$mode.'&amp;select_acc='.$select_acc);
*/
	}
	else{
...
/*
    ...
    $currentgroup = setup_and_print_groups($course, $groupmode, $CFG->wwwroot . '/mod/referentiel/export_task.php?d='.$referentiel->id.'&amp;mode='.$mode.'&amp;select_acc='.$select_acc);
*/
	}
	print_heading(format_string($referentiel->name));
	/// Print the tabs
	if (!isset($mode)){
		$mode='importstudent'; // un seul mode possible
	}
	$currenttab = $mode;
    ...
    print_heading_with_help($strmessage, 'importstudents', 'referentiel', $icon);
    ...
    if (!empty($format)) { 
        ...
        if (!empty($params->choosefile)) {
			...
			// DEBUG
			// echo "<br />DEBUG :: import_student.php :: 209 :: <br />{$CFG->dataroot}/{$course->id}/{$params->choosefile}\n";
			// exit;
        ...
        if ($fileisgood) { 
            ...
            $eformat->setRReferentiel( $referentiel );			
            $eformat->setRReferentiel( $referentiel_referentiel );
            $eformat->setCourse( $course );
			$eformat->setCoursemodule( $cm); 
            $eformat->setFilename( $importfile );
            $eformat->setStoponerror( $params->stoponerror );
			$eformat->setOverride( $params->override );
			$eformat->setReturnpage("");
            // Do anything before that we need to
            if (! $eformat->importpreprocess()) { 
                print_error( $txt->importerror ,
                      "$CFG->wwwroot/mod/referentiel/import_student.php?d={$referentiel->id}");
            }
    ...
    <form id="form" enctype="multipart/form-data" method="post" action="<?php p("import_student.php?d=$referentiel->id") ?>">
        <fieldset class="invisiblefieldset" style="display: block;">
            ...
            <table cellpadding="5">
                <tr>
                    ...
                    <td><?php choose_from_menu($fileformatnames, 'format', 'csv', '');
