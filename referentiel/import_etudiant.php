<?php  // $Id: import_etudiant.php,v 1.1 2010/12/17 21:49:35 vf Exp $
///////////////////////////////////////////////////////////////////////////
//                                                                       //
// NOTICE OF COPYRIGHT                                                   //
//                                                                       //
...
*/
	...
	elseif ($id) {
		...
		error(get_string('erreurscript','referentiel','Erreur01 : import_etudiant.php'));
	}
	...
	$currentgroup = setup_and_print_groups($course, $groupmode, 'import_etudiant.php?d='.$referentiel->id);
...
//		update_module_button($cm->id, $course->id, get_string('modulename', 'referentiel')), // HTML code for a button (usually for module editing)
    ...
    print_heading_with_help($strmessage, 'importetudiants', 'referentiel', $icon);
    ...
    if (!empty($format)) { 
        ...
        if (!empty($params->choosefile)) {
			...
			// DEBUG
			// echo "<br />DEBUG :: import_etudiant.php :: 209 :: <br />{$CFG->dataroot}/{$course->id}/{$params->choosefile}\n";
			// exit;
        ...
        if ($fileisgood) { 
            ...
            if (! $eformat->importpreprocess()) { 
                error( $txt->importerror ,
                      "$CFG->wwwroot/mod/referentiel/import_etudiant.php?d={$referentiel->id}");
            }
    ...
    <form id="form" enctype="multipart/form-data" method="post" action="<?php p("import_etudiant.php?d=$referentiel->id") ?>">
