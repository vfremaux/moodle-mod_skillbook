<?php  // $Id: export_etudiant.php,v 1.1 2010/12/17 21:49:31 vf Exp $
///////////////////////////////////////////////////////////////////////////
//                                                                       //
// NOTICE OF COPYRIGHT                                                   //
//                                                                       //
...
*
* @version $Id: export_etudiant.php,v 1.1 2010/12/17 21:49:31 vf Exp $
...
*/
	...
	elseif ($id) {
		...
		error(get_string('erreurscript','referentiel','Erreur01 : export_etudiant.php'));
	}
	
	if ($etudiant_id) { // id etudiant
        if (! $record = get_record('referentiel_etudiant', 'id', $etudiant_id)) {
            error('etudiant ID is incorrect');
        }
	}
	...
	$currentgroup = setup_and_print_groups($course, $groupmode, 'export_etudiant.php?d='.$referentiel->id);
...
//		update_module_button($cm->id, $course->id, get_string('modulename', 'referentiel')), // HTML code for a button (usually for module editing)
    ...
    print_heading_with_help($strmessage, 'exportetudiant', 'referentiel', $icon);
...
// echo "<br /> OK 7\n";
        ...
        if (! $eformat->exportpreprocess()) {   // Do anything before that we need to
            error( $txt->exporterror, $CFG->wwwroot.'/mod/referentiel/export_etudiant.php?id='.$cm->id);
        }
// echo "<br /> OK 8\n";
...
// echo "<br /> OK 9\n";
        if (! $eformat->exportpostprocess()) {                    // In case anything needs to be done after
            error( $txt->exporterror, $CFG->wwwroot.'/mod/referentiel/export_etudiant.php?d='.$cm->id);
        }
        echo "<hr />";
// echo "<br /> OK 10\n";
